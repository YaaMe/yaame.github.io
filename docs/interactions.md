...
| | Workers 上 | node 上 |
|---|---|---|
| 实现 | D1(第一方绑定) | 内置 `node:sqlite`,零依赖 |
...
没有 FTS、没有 SQLite 专有函数。这样真要换 Postgres 几乎是逐句照搬。

三条免费的防护:

- 时间一律存 ISO 字符串或整数时间戳,不用任何方言相关的日期类型
- 不用 SQLite 专有函数
- 所有语句集中在一个文件,不散落在各处
...
- [ ] `platform.sql` 这道缝 + D1 与 `node:sqlite` 两个实现
- [ ] schema:活动 id 唯一约束、`inReplyTo`、作者、内容、时间、`deleted_at`、`promoted_at`
- [ ] `.on(Create)` —— 现在唯一还在丢数据的地方:别人回复了,活动进来就被丢掉
- [ ] `.on(Delete)` 补上对象删除(现在只处理账号注销)
- [ ] 提升命令与 filter
- [ ] 每周的 PR job
- [ ] 路由层与 OAuth
